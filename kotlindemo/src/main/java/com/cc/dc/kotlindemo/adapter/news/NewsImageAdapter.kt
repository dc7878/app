 ...
 * news image adapter
 */
class NewsImageAdapter : AbsListItemAdapterDelegate<NewsImageEntity, NewsEntity, CommonViewHolder> {
    ...
    override fun onBindViewHolder(item: NewsImageEntity, viewHolder: CommonViewHolder, payloads: MutableList<Any>) {
    }

    override fun onCreateViewHolder(parent: ViewGroup): CommonViewHolder {
        return CommonViewHolder(inflater!!.inflate(R.layout.item_news_image, parent, false))
    }

    override fun isForViewType(item: NewsEntity, items: MutableList<NewsEntity>, position: Int): Boolean {
        return  item is NewsImageEntity
    }
}